...
class PadFoundation:
    ...
    def __init__(self, foundation_length: float, foundation_width: float, column_length: float, column_width: float, col_pos_xdir : float, col_pos_ydir:float, soil_bearing_capacity = 150):
        '''
        ...
        assert_maximum_input_limit(col_pos_xdir, foundation_length, 'column position x direction')
        assert_maximum_input_limit(col_pos_ydir, foundation_width, 'column position y direction')
    ...
    def plot_geometry(self):
        ...
        x_col = [self.col_pos_xdir-self.column_length/2, self.col_pos_xdir+self.column_length/2,
                 self.col_pos_xdir+self.column_length /
                 2, self.col_pos_xdir-self.column_length/2,
                 self.col_pos_xdir - self.column_length/2]
        y_col = [self.col_pos_ydir - self.column_width/2, self.col_pos_ydir-self.column_width/2,
                 self.col_pos_ydir+self.column_width /
                 2, self.col_pos_ydir+self.column_width/2,
                 self.col_pos_ydir-self.column_width/2]
        fig_plan.add_trace(go.Scatter(x=x, y=y, name='FOOTING',
                           mode='lines', line=dict(color='red', width=3)))
        fig_plan.add_trace(go.Scatter(x=x_col, y=y_col, name='COLUMN',
                           mode='lines', line=dict(color='green', width=1.5)))

        fig_plan.update_layout(title_text='FOOTING PLAN',
                               width=500, height=500, showlegend=True)
        fig_plan.show()

    def foundation_loads(self, foundation_thickness=300, soil_depth_abv_foundation=500, soil_unit_weight=18, concrete_unit_weight=24):
        '''
        ...
        # data validation for inputs
        assert_input_limit(foundation_thickness, self.foundation_thickness * 1000,
                           'Pad foundation thickness')
        assert_input_limit(soil_depth_abv_foundation, self.soil_depth_abv_foundation * 1000,
    ...
    def column_axial_loads(self, permanent_axial_load=0, imposed_axial_load=0, wind_axial_load=0):
        '''
    ...
    def column_horizontal_loads_xdir(self, permanent_horizontal_load_xdir=0, imposed_horizontal_load_xdir=0, wind_horizontal_load_xdir=0):
        '''
        ...
        # data validation for inputs
        assert_number(permanent_horizontal_load_xdir,
                      'Permanent Horizontal loads in X direction')
        assert_number(imposed_horizontal_load_xdir,
                      'Imposed Horizontal loads in X direction')
        assert_number(wind_horizontal_load_xdir,
                      'Wind Horizontal loads in X direction')

    def column_horizontal_loads_ydir(self, permanent_horizontal_load_ydir=0, imposed_horizontal_load_ydir=0, wind_horizontal_load_ydir=0):
        '''
        ...
        # data validation for inputs
        assert_number(permanent_horizontal_load_ydir,
                      'Permanent Horizontal loads in Y direction')
        assert_number(imposed_horizontal_load_ydir,
                      'Imposed Horizontal loads in Y direction')
        assert_number(wind_horizontal_load_ydir,
                      'Wind Horizontal loads in Y direction')

    def column_moments_xdir(self, permanent_moment_xdir=0, imposed_moment_xdir=0, wind_moments_xdir=0):
        '''
        Accepts the column moments along the X direction. This comprises of permanent,imposed and wind moments
        ...
        # data validation for inputs
        assert_number(permanent_moment_xdir,
                      'Permanent moments in X direction')
    ...
    def column_moments_ydir(self, permanent_moment_ydir=0, imposed_moment_ydir=0, wind_moments_ydir=0):
        '''
        ...
        # data validation for inputs
        assert_number(permanent_moment_ydir,
                      'Permanent moments in Y direction')
    ...
    def total_force_X_dir_sls(self):
        ...
        '''
        return self.permanent_horizontal_load_xdir + self.imposed_horizontal_load_xdir + self.wind_horizontal_load_xdir
    ...
    def total_force_Y_dir_sls(self):
        ...
        '''
        return self.permanent_horizontal_load_ydir + self.imposed_horizontal_load_ydir + self.wind_horizontal_load_ydir
    ...
    def total_force_Z_dir_sls(self):
        ...
        fdn_loads = self.foundation_loads(
            self.foundation_thickness * 1000, self.soil_depth_abv_foundation * 1000, self.soil_unit_weight, self.concrete_unit_weight)
        return self.permanent_axial_load + self.imposed_axial_load + self.wind_axial_load + (self.area_of_foundation() * (fdn_loads[0] + fdn_loads[1]))
    ...
    def total_moments_X_direction_sls(self):
        ...
        fdn_loads = self.foundation_loads(
            self.foundation_thickness * 1000, self.soil_depth_abv_foundation * 1000, self.soil_unit_weight, self.concrete_unit_weight)
        Mdx = (self.area_of_foundation() * (fdn_loads[0] + fdn_loads[1]) * (self.foundation_length/2)) \
            + (self.permanent_axial_load * self.col_pos_xdir) + self.permanent_moment_xdir + \
            (self.permanent_horizontal_load_xdir * self.foundation_thickness) + (self.imposed_axial_load * self.col_pos_xdir) \
            + self.imposed_moment_xdir + (self.imposed_horizontal_load_xdir * self.foundation_thickness) \
            + (self.wind_axial_load * self.col_pos_xdir) + self.wind_moments_xdir + \
    ...
    def total_moments_Y_direction_sls(self):
        ...
        fdn_loads = self.foundation_loads(
            self.foundation_thickness * 1000, self.soil_depth_abv_foundation * 1000, self.soil_unit_weight, self.concrete_unit_weight)
        Mdy = self.area_of_foundation() * (fdn_loads[0] + fdn_loads[1]) * self.foundation_width/2\
            + self.permanent_axial_load * self.col_pos_ydir + self.permanent_moment_ydir + \
            self.permanent_horizontal_load_ydir * self.foundation_thickness + self.imposed_axial_load * self.col_pos_ydir\
            + self.imposed_moment_ydir + self.imposed_horizontal_load_ydir * self.foundation_thickness \
            + self.wind_axial_load * self.col_pos_ydir + self.wind_moments_ydir + \
    ...
    def eccentricity_X_direction_sls(self):
        ...
        '''
        ex = (self.total_moments_X_direction_sls() /
              self.total_force_Z_dir_sls()) - (self.foundation_length/2)
    ...
    def eccentricity_Y_direction_sls(self):
        ...
        '''
        ey = (self.total_moments_Y_direction_sls() /
              self.total_force_Z_dir_sls()) - (self.foundation_width/2)
    ...
    def pad_base_pressures_sls(self):
        ...
        """
        q1 = self.total_force_Z_dir_sls() * ((1 - (6 * (self.eccentricity_X_direction_sls() / (self.foundation_length*1000))
                                                   ) - (6 * (self.eccentricity_Y_direction_sls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q2 = self.total_force_Z_dir_sls() * ((1 - (6 * (self.eccentricity_X_direction_sls() / (self.foundation_length*1000))
                                                   ) + (6 * (self.eccentricity_Y_direction_sls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q3 = self.total_force_Z_dir_sls() * ((1 + (6 * (self.eccentricity_X_direction_sls() / (self.foundation_length*1000))
                                                   ) - (6 * (self.eccentricity_Y_direction_sls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q4 = self.total_force_Z_dir_sls() * ((1 + (6 * (self.eccentricity_X_direction_sls() / (self.foundation_length*1000))
                                                   ) + (6 * (self.eccentricity_Y_direction_sls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q1 = round(q1, 3)
        q2 = round(q2, 3)
        q3 = round(q3, 3)
    ...
    def bearing_pressure_check_sls(self):
        ...
        if (minimum_pad_pressure <= self.soil_bearing_capacity ) and (maximum_pad_pressure <= self.soil_bearing_capacity):
            status = print(
    ...
    def plot_base_pressures_sls(self):
        ...
        fig_plan.add_trace(go.Scatter(x=x, y=y, name='FOOTING',
                           mode='lines+text', line=dict(color='red', width=1.5), text=new_pressure_text, textposition="top center"))
        fig_plan.add_trace(go.Scatter(x=x_col, y=y_col, name='COLUMN',
                           mode='lines', line=dict(color='green', width=1.5)))
    ...
    def total_force_X_dir_uls(self):
        ...
        float
            sum of permanent,imposed and wind horizontal loads in x direction in kN
        '''
        total = (self.uls_strength_factor_permanent * self.permanent_horizontal_load_xdir) + (self.imposed_horizontal_load_xdir *
                                                                                              self.uls_strength_factor_imposed) + (self.wind_horizontal_load_xdir * self.uls_strength_factor_imposed)
    ...
    def total_force_Y_dir_uls(self):
        ...
        float
            sum of permanent,imposed and wind horizontal loads in y direction in kN
        '''
        total = (self.uls_strength_factor_permanent * self.permanent_horizontal_load_ydir) + (self.imposed_horizontal_load_ydir *
                                                                                              self.uls_strength_factor_imposed) + (self.wind_horizontal_load_ydir * self.uls_strength_factor_imposed)
        return round(total, 3)
    ...
    def total_force_Z_dir_uls(self):
        ...
        fdn_loads = self.foundation_loads(
            self.foundation_thickness * 1000, self.soil_depth_abv_foundation * 1000, self.soil_unit_weight, self.concrete_unit_weight)
        total = (self.uls_strength_factor_imposed * self.imposed_axial_load) + (self.uls_strength_factor_imposed*self.wind_axial_load) + \
            self.uls_strength_factor_permanent * ((self.permanent_axial_load ) + (self.area_of_foundation() * (fdn_loads[0] + fdn_loads[1])))
        return round(total, 3)
    ...
    def total_moments_X_direction_uls(self):
        ...
        Mdx = self.uls_strength_factor_permanent * (self.area_of_foundation() * (fdn_loads[0] + fdn_loads[1]) * self.foundation_length/2 + (self.permanent_axial_load * self.col_pos_xdir)) \
            + self.uls_strength_factor_permanent * (self.permanent_moment_xdir) \
            + self.uls_strength_factor_imposed * self.imposed_axial_load * self.col_pos_xdir \
            + self.uls_strength_factor_imposed * self.imposed_moment_xdir \
            + self.uls_strength_factor_imposed * self.wind_axial_load * self.col_pos_xdir  \
            + self.uls_strength_factor_imposed * self.wind_moments_xdir \
            + (self.uls_strength_factor_permanent * self.permanent_horizontal_load_xdir + self.uls_strength_factor_imposed *
               self.imposed_horizontal_load_xdir + self.uls_strength_factor_imposed * self.wind_horizontal_load_xdir) * self.foundation_thickness
        return round(Mdx, 3)
    ...
    def total_moments_Y_direction_uls(self):
        ...
        Mdy = self.uls_strength_factor_permanent * (self.area_of_foundation() * (fdn_loads[0] + fdn_loads[1]) * self.foundation_width/2 + (self.permanent_axial_load * self.col_pos_ydir)) \
            + self.uls_strength_factor_permanent * self.permanent_moment_ydir \
            + self.uls_strength_factor_imposed * self.imposed_axial_load * self.col_pos_ydir \
            + self.uls_strength_factor_imposed_unfavourable * self.imposed_moment_ydir \
            + self.uls_strength_factor_imposed * self.wind_axial_load * self.col_pos_ydir \
            + self.uls_strength_factor_imposed * self.wind_moments_ydir \
            + (self.uls_strength_factor_permanent * self.permanent_horizontal_load_ydir + self.uls_strength_factor_imposed *
               self.imposed_horizontal_load_ydir + self.uls_strength_factor_imposed * self.wind_horizontal_load_ydir) * self.foundation_thickness
        return round(Mdy, 3)
    ...
    def eccentricity_X_direction_uls(self):
        ...
        float
            the eccentricity of the foundation in x direction units are in mm.
        '''
        ex = (self.total_moments_X_direction_uls() / self.total_force_Z_dir_uls()) - (self.foundation_length/2)
    ...
    def eccentricity_Y_direction_uls(self):
        ...
        float
            the eccentricity of the foundation in y direction units are in mm.
        '''
        ey = (self.total_moments_Y_direction_uls() / self.total_force_Z_dir_uls()) - (self.foundation_width/2)
    ...
    def pad_base_pressures_uls(self):
        ...
        q4 : float
            base pressure at sls located at the top right corner of the pad foundation in kN/m2
        """
        q1 = self.total_force_Z_dir_uls() * ((1 - (6 * (self.eccentricity_X_direction_uls() / (self.foundation_length*1000))
                                                   ) - (6 * (self.eccentricity_Y_direction_uls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q2 = self.total_force_Z_dir_uls() * ((1 - (6 * (self.eccentricity_X_direction_uls() / (self.foundation_length*1000))
                                                   ) + (6 * (self.eccentricity_Y_direction_uls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q3 = self.total_force_Z_dir_uls() * ((1 + (6 * (self.eccentricity_X_direction_uls() / (self.foundation_length*1000))
                                                   ) - (6 * (self.eccentricity_Y_direction_uls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q4 = self.total_force_Z_dir_uls() * ((1 + (6 * (self.eccentricity_X_direction_uls() / (self.foundation_length*1000))
                                                   ) + (6 * (self.eccentricity_Y_direction_uls()/(self.foundation_width*1000))))/self.area_of_foundation())
        q1 = round(q1, 3)
        q2 = round(q2, 3)
        q3 = round(q3, 3)
    ...
    def base_pressure_rate_of_change_X(self):
        ...
        """
        left_hand_base_reaction = (self.pad_base_pressures_uls()[0] + self.pad_base_pressures_uls()[1]) * self.foundation_width/2
        right_hand_base_reaction =  (self.pad_base_pressures_uls()[2] + self.pad_base_pressures_uls()[3]) * self.foundation_width/2
    ...
    def base_pressure_rate_of_change_Y(self):
        ...
        """
        top_edge_base_reaction = (self.pad_base_pressures_uls()[3] + self.pad_base_pressures_uls()[1]) * self.foundation_length/2
        bottom_edge_base_reaction =  (self.pad_base_pressures_uls()[0] + self.pad_base_pressures_uls()[2]) * self.foundation_length/2
...
class padFoundationDesign(PadFoundation):
    ...
    '''
    def __init__(self,PadFoundation,fck : float = 25,fyk : float = 460,concrete_cover : float = 30,bar_diameterX : int =16,bar_diameterY:int=16):
        '''
        ...
        self.dx = ((self.PadFoundation.foundation_thickness*1000)- concrete_cover - (bar_diameterX/2))/1000
        self.dy = ((self.PadFoundation.foundation_thickness*1000)- concrete_cover - (bar_diameterY/2) - bar_diameterX)/1000
        self.beta = 0
        ...
        assert_input_range('bar_diameterX',bar_diameterX,[8,10,12,16,20,25,32,40])
        assert_input_range('bar_diameterY',bar_diameterY,[8,10,12,16,20,25,32,40])
    ...
    def __loading_diagrams_X_dir(self):
        ...
        fdn_loads = self.foundation_loads(
            self.PadFoundation.foundation_thickness * 1000, self.PadFoundation.soil_depth_abv_foundation * 1000, self.PadFoundation.soil_unit_weight, self.PadFoundation.concrete_unit_weight)
        udl = ((sum(fdn_loads)) * self.PadFoundation.foundation_width*1000)*self.PadFoundation.uls_strength_factor_permanent
        if left_load != right_load:
            foundation_load1 = TrapezoidalLoadV(force=(left_load,right_load),span=(0,self.PadFoundation.foundation_length))
        else:
            foundation_load1 = DistributedLoadV(left_load,span=(0,self.PadFoundation.foundation_length))
        foundation_load2 = DistributedLoadV(-udl,span=(0,self.PadFoundation.foundation_length))
    ...
    def __loading_diagrams_Y_dir(self):
        ...
        fdn_loads = self.foundation_loads(
            self.PadFoundation.foundation_thickness * 1000, self.PadFoundation.soil_depth_abv_foundation * 1000, self.PadFoundation.soil_unit_weight, self.PadFoundation.concrete_unit_weight)
        udl = ((sum(fdn_loads)) * self.PadFoundation.foundation_length*1000) * self.PadFoundation.uls_strength_factor_permanent
        if left_load != right_load:
            foundation_load1 = TrapezoidalLoadV(force=(left_load,right_load),span=(0,self.PadFoundation.foundation_width))
        else:
            foundation_load1 = DistributedLoadV(left_load,span=(0,self.PadFoundation.foundation_width))
        foundation_load2 = DistributedLoadV(-udl,span=(0,self.PadFoundation.foundation_width))
        foundation.add_loads(foundation_load1,foundation_load2)
        return foundation
    ...
    def plot_shear_force_X(self):
        ...
        foundation = self.__loading_diagrams_X_dir()
        foundation.analyse()
        x1 = self.PadFoundation.col_pos_xdir - (self.PadFoundation.column_length/2 + self.dx)
        x2 = self.PadFoundation.col_pos_xdir + self.PadFoundation.column_length/2 + self.dx
    ...
    def plot_shear_force_Y(self):
        ...
        foundation = self.__loading_diagrams_Y_dir()
        foundation.analyse()
        y1 = self.PadFoundation.col_pos_ydir - (self.PadFoundation.column_width/2  + self.dy)
        y2 = self.PadFoundation.col_pos_ydir + self.PadFoundation.column_width/2 + self.dy
    ...
    def area_of_steel_reqd_X_dir(self):
        ...
        area_of_steel_required_calc = bending_reinforcement(Med,self.dx,self.fck,self.fyk,self.PadFoundation.foundation_width * 1000)
        area_of_steel_required = max(area_of_steel_required_calc,minimum_steel(self.fck,self.fyk,self.PadFoundation.foundation_width * 1000,self.dx))
        area_required_per_m = area_of_steel_required/self.PadFoundation.foundation_width
    ...
    def area_of_steel_reqd_Y_dir(self):
        ...
        area_of_steel_required_calc = bending_reinforcement(Med,self.dy,self.fck,self.fyk,self.PadFoundation.foundation_length * 1000)
        area_of_steel_required = max(area_of_steel_required_calc,minimum_steel(self.fck,self.fyk,self.PadFoundation.foundation_length * 1000,self.dy))
        area_required_per_m = area_of_steel_required/self.PadFoundation.foundation_length
        return round(area_required_per_m)
    ...
    def tranverse_shear_check_Xdir(self):
        ...
        px = round((self.__reinforcement_calculations_X_dir()[3])/(1000*self.dx*1000),5)
        vrd_c = shear_stress_check_1d(self.dx*1000,px,self.fck)
        Vrd_c = round((vrd_c * self.PadFoundation.foundation_width*1000 * self.dx*1000)/1000,3)
    ...
    def tranverse_shear_check_Ydir(self):
        ...
        py =  round((self.__reinforcement_calculations_Y_dir()[3])/(1000*self.dx*1000),5)
        vrd_c = shear_stress_check_1d(self.dy*1000,py,self.fck)
        Vrd_c = (vrd_c * self.PadFoundation.foundation_length*1000 * self.dx*1000)/1000
        if Vrd_c > design_shear_force:
    ...
    def __punching_shear(self):
        ...
        px = round((self.__reinforcement_calculations_X_dir()[3])/(1000*self.dx*1000),5)
        py = round((self.__reinforcement_calculations_Y_dir()[3])/(1000*self.dy*1000),5)
    ...
    def punching_shear_column_face(self):
        ...
        '''
        column_perimeter = 2*(self.PadFoundation.column_length+self.PadFoundation.column_width)
        design_punching_shear_force = (self.PadFoundation.uls_strength_factor_permanent * self.PadFoundation.permanent_axial_load)\
             +   (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.imposed_axial_load)
        punching_shear_stress = design_punching_shear_force / (column_perimeter * np.average([self.dy,self.dx])*1000)
    ...
    def punching_shear_check_1d(self):
        ...
        string
            maximum punching shear resistance in N/mm2 and design punching shear stress in N/mm2 
        '''
        cx = (self.PadFoundation.base_pressure_rate_of_change_X()[1] - self.PadFoundation.base_pressure_rate_of_change_X()[0])/self.PadFoundation.foundation_length
        cy = (self.PadFoundation.base_pressure_rate_of_change_Y()[0] - self.PadFoundation.base_pressure_rate_of_change_Y()[1])/self.PadFoundation.foundation_width
        ...
        eccxa = self.PadFoundation.col_pos_xdir - self.PadFoundation.foundation_length/2
        eccya = self.PadFoundation.col_pos_ydir - self.PadFoundation.foundation_width/2
        ult_pressure_punching_shear = soil_pressure_ult1 + ((self.PadFoundation.foundation_length/2 + eccxa - self.PadFoundation.column_length/2 - average_depth) + 0.5*(self.PadFoundation.foundation_length+2*average_depth)) * cx/self.PadFoundation.foundation_width\
            - ((self.PadFoundation.foundation_width/2 + eccya - self.PadFoundation.column_width/2 - average_depth) + 0.5*(self.PadFoundation.foundation_width+2*average_depth)) * cy/self.PadFoundation.foundation_length
        ...
        column_axial_load =  (self.PadFoundation.uls_strength_factor_permanent * self.PadFoundation.permanent_axial_load)\
                +   (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.imposed_axial_load)
        design_punching_shear_force = column_axial_load + ((fdn_loads_ult/self.PadFoundation.area_of_foundation()) - ult_pressure_punching_shear ) * area
        column_ult_moment_Xdir = (self.PadFoundation.uls_strength_factor_permanent * self.PadFoundation.permanent_moment_xdir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.imposed_moment_xdir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.wind_moments_xdir)
        column_ult_moment_Ydir = (self.PadFoundation.uls_strength_factor_permanent * self.PadFoundation.permanent_moment_ydir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.imposed_moment_ydir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.wind_moments_ydir)
        design_punching_shear_force_eff = design_punching_shear_force * (1+1*abs(column_ult_moment_Xdir)/design_punching_shear_force*self.PadFoundation.column_width + 1 * abs(column_ult_moment_Ydir)/design_punching_shear_force*self.PadFoundation.column_length)
        ved = ((design_punching_shear_force_eff*1000)/(perimeter_length*average_depth*1000000)) 
        if self.PadFoundation.eccentricity_X_direction_uls() == 0 and self.PadFoundation.eccentricity_Y_direction_uls() == 0:
            ved_design = ved
        elif self.PadFoundation.eccentricity_X_direction_uls() != 0 or self.PadFoundation.eccentricity_Y_direction_uls() != 0:
            column_ratio = c1/c2
            if column_ratio <= 0.5:
                column_ratio = 0.5
            elif column_ratio >= 3:
            ...
            W = c1 * c2 + (2*c2*average_depth) + (0.5 * c1**2) + (4 * average_depth**2) + (math.pi * c1 * average_depth)
            foundation_moments = abs(column_ult_moment_Xdir + column_ult_moment_Ydir)
            if self.beta == 0:
                beta = 1 + (k * (foundation_moments/design_punching_shear_force_eff) * (perimeter_length/W))
            elif self.beta != 0:
                beta = self.beta
            ved_design = ved * beta
    ...
    def punching_shear_check_2d(self):
        ...
        eccxa = self.PadFoundation.col_pos_xdir - self.PadFoundation.foundation_length/2
        eccya = self.PadFoundation.col_pos_ydir - self.PadFoundation.foundation_width/2
        ult_pressure_punching_shear = soil_pressure_ult1 + ((self.PadFoundation.foundation_length/2 + eccxa - self.PadFoundation.column_length/2 - 2*average_depth) + 0.5*(self.PadFoundation.foundation_length+4*average_depth)) * cx/self.PadFoundation.foundation_width\
            - ((self.PadFoundation.foundation_width/2 + eccya - self.PadFoundation.column_width/2 - 2*average_depth) + 0.5*(self.PadFoundation.foundation_width+4*average_depth)) * cy/self.PadFoundation.foundation_length
        area = ((c1 * c2 )+ 2*(c1+c2) * 2*average_depth + (math.pi * (2*average_depth)**2)) 
        ...
        column_ult_moment_Xdir = (self.PadFoundation.uls_strength_factor_permanent * self.PadFoundation.permanent_moment_xdir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.imposed_moment_xdir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.wind_moments_xdir)
        column_ult_moment_Ydir = (self.PadFoundation.uls_strength_factor_permanent * self.PadFoundation.permanent_moment_ydir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.imposed_moment_ydir) + (self.PadFoundation.uls_strength_factor_imposed * self.PadFoundation.wind_moments_ydir)
        design_punching_shear_force_eff = design_punching_shear_force * (1+2*abs(column_ult_moment_Xdir)/design_punching_shear_force*self.PadFoundation.column_width + 2 * abs(column_ult_moment_Ydir)/design_punching_shear_force*self.PadFoundation.column_length)
        ved = ((design_punching_shear_force_eff*1000)/(perimeter_length*average_depth*1000000)) 
        if self.PadFoundation.eccentricity_X_direction_uls() == 0 and self.PadFoundation.eccentricity_Y_direction_uls() == 0:
            ved_design = ved
        elif self.PadFoundation.eccentricity_X_direction_uls() != 0 or self.PadFoundation.eccentricity_Y_direction_uls() != 0:
            ...
            if column_ratio <= 0.5:
                ...
                column_ratio = 3
            k = column_punching_coefficient_k(column_ratio)
            W = c1 * c2 + (2*c2*2*average_depth) + (0.5 * c1**2) + (4 * (2*average_depth)**2) + (math.pi * c1 * 2*average_depth)
            foundation_moments = abs(column_ult_moment_Xdir + column_ult_moment_Ydir)
            if self.beta == 0:
                beta = 1 + (k * (foundation_moments/design_punching_shear_force_eff) * (perimeter_length/W))
            elif self.beta != 0:
